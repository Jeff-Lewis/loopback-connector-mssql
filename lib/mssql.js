 ...
 * Parsing taken from Polyfill https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/Date/toISOString
 * @param val
 */
function dateToMsSql(val) {
  ...
  // Default to useUTC true if undefined.
  if(this.settings.useUTC !== false){
    dateStr = val.getUTCFullYear() +
        ...
        '.' + (val.getUTCMilliseconds() / 1000).toFixed(3).slice(2, 5) +
        'Z';
  }else {
    ...
    ':' + pad(val.getMinutes()) +
    ':' + pad(val.getSeconds()) +
    '.' + (val.getMilliseconds() / 1000).toFixed(3).slice(2, 5) +
    'Z';
  }
